...
class Channel(YoutubeObject):
    ...
    @classmethod
    def from_title(cls, session, title: str):
        """
    ...
    @classmethod
    def from_username(cls, session, username: str, cache_only: bool = False):
        """
        ...
        :raises: AssertionError if more or less than 1 channel is returned from the API
        """
        if cached := session.query(cls).filter(cls.username == username).first():
            return cached
        if cache_only:
            return
        ...
        if cached_by_id := session.get(cls, channels[0]['id']):
    ...
    @classmethod
    def from_url(cls, session, url, cache_only=False):
        """
        ...
        """
        if UrlLookup.url_is_username(session, url):
            return cls.from_username(session, UrlLookup.get_resolved(session, url), cache_only=cache_only)

        url = UrlLookup.get_resolved(session, url) or url
        if cached := session.query(cls).filter(cls.url == url).first():
            return cached
        if cache_only:
            return
        ...
        if response.status_code == 200:
            ...
            if og_url := soup.find('meta', property='og:url'):
                if "/user/" in response.url:
                    channel = cls.from_username(session, response.url.split("/")[-1])
                    session.add(UrlLookup(original=url, resolved=response.url.split("/")[-1], is_username=True))
                else:
                    channel = cls.from_id(session, og_url['content'].split("/")[-1])
                if channel.url:
                    session.add(UrlLookup(original=url, resolved=channel.url, is_username=False))
                session.commit()
                return channel
            else:
