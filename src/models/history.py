...
class History(Base):
    ...
    @classmethod
    def add(cls, session, channel):
        if old := session.query(cls).filter(cls.channel_id == channel.id).first():
            old.popularity += 1
    ...
    @classmethod
    def get(cls, session):
        return session.query(cls).order_by(cls.popularity.desc()).limit(10).all()
